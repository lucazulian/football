...
exec-in-docker = docker-compose exec football
...
shell: container-football			## Enter into football service
	${exec-in-docker} bash
...
delete: halt						## Delete all containers, images and volumes
	@docker images -a | grep "football" | awk '{print $3}' | xargs docker rmi -f | docker ps -a | grep "football" | awk '{print $1}' | xargs docker rm -v
